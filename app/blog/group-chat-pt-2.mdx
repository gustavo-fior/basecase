...
both looks and feels exactly like imessage. in [part 1](/blog/group-chat), i walked through how i
built the backend to make the ai personas chat autonomously and mimic the dynamics of a real group
chat. in this post, i want to switch gears and focus on the frontend.
...
happening under the hood. each detail requires a good amount of front-end trickery, from carefully
placed gradients and svg masks to rich text editors and css animations.
...
let’s walk through how they work together to achieve the imessage look and feel.
...
5. the reactions are a bit trickier. each reaction icon is masked via `webkitmaskimage` or
   `maskimage`, then filled with the fixed gradient. here’s an example:
...
the result is that each reaction bubble displays the same fixed gradient behind the scenes, giving
them the same color variation as the main message bubbles. without the pseudo elements,
...
effect: user messages look light at the top, deeper blue at the bottom, and all the reaction icons
are seamlessly tinted to match.
...
i’m pretty happy with how it turned out, but there’s still room for improvement. if you spot
something i could do better or have an alternative approach to any of these effects, i’d love to
hear about it. reach out to me on [x](https://x.com/alanagoyal) or submit a pull request on
